...
func (c *SshegoConfig) DefineFlags(fs *flag.FlagSet) {
	...
	fs.StringVar(&c.LocalToRemote.Listen.Addr, "listen", "", "(forward tunnel) We listen on this host:port locally, securely tunnel that traffic to sshd, then send it cleartext to -remote. The forward tunnel is active if and only if -listen is given. If port starts with a '/' then we treat it as the path to a unix-domain socket to listen on.")
	fs.StringVar(&c.LocalToRemote.Remote.Addr, "remote", "", "(forward tunnel) After traversing the secured forward tunnel, -listen traffic flows in cleartext from the sshd to this host:port. The foward tunnel is active only if -listen is given too.  If port starts with a '/' then we treat it as the path to a unix-domain socket to forward to.")
